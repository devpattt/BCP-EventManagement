    ...
    if ($stmt->execute()) {
        if ($new_status == 'Cancelled' || $new_status == 'Approved') {
            ...
            if ($event_result->num_rows > 0) {
                ...
                if (!is_null($event_data['date_booked']) && !empty($event_data['date_booked'])) {
                    $date_booked = date("Y-m-d", strtotime($event_data['date_booked']));
                    ...
                    $history_stmt->bind_param("sssiiss", $event_data['name'], $event_data['contact'], $event_data['event_title'], $event_data['attendees'], $date_booked, $event_data['time'], $new_status);
                    ...
                    echo "Error: date_booked is null or empty."; 
                }
            }
            $event_stmt->close();
        }
